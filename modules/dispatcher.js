...
async function processTransaction(currAddy, results, totalAmount) {
	...
	let addysStr = getAddys(results) + "," + newAddy;
	await saveNewTransaction(tx, response, addysStr, totalAmount);
...
async function saveNewAddy(newAddy, oldAddy) {
	...
	return await db.saveAddy(addyObj);
}

function getAddys(result) {
	let addys = [];
	for (let i = result.length - 1; i >= 0; i--) {
		addys.push(result[i].addy);
	}
	return addys.join(',');
}
...
async function saveNewTransaction(tx, resp, addys, value) {
	...
	try {
		...
		// Continue without throwing to match original behavior
	}
}
